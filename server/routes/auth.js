...
export const getAccessToken = (req, res) => {
	...
	axios({
		...
		.then((response) => {
			...
			const getGeniusAccountURL = `${process.env.NODE_ENV === 'development'
				? 'http://localhost:3333'
				: 'https://rap-clouds-server.herokuapp.com'}/getGeniusAccount`;
...
const getGeniusAccount = async (req, res, next) => {
	...
	try {
		...
		if (data) {
			...
			res.status(200).sendFile(path.resolve('public/signedIn.html'));
		}
	} catch (err) {
